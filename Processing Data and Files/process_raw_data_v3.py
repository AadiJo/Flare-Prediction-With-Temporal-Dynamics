...
def create_final_dataset(processed_data):
    ...
    metadata = []
    sharp_metadata = []  # New array for SHARP metadata
    ...
    for i, item in enumerate(tqdm(processed_data, desc="Creating arrays")):
        ...
        sharp_metadata.append(item['sharp_metadata'])  # Store SHARP metadata
    ...
    return X_array, X_original_array, y_array, metadata, sharp_metadata
...
def process_fits_file(fits_file_path):
    try:
        with fits.open(fits_file_path) as hdul:
            ...
            # Extract SHARP keywords from header
            sharp_values = {key: header.get(key, None) for key in SHARP_KEYWORDS}
            # Return both normalized and original resized data, plus SHARP metadata
            return normalize_image_fast(image_resized), image_resized, sharp_values
    except Exception as e:
...
def process_timestep_optimized(timestep_folder, available_segments):
    try:
        ...
        with ThreadPoolExecutor(max_workers=min(4, len(available_segments))) as executor:
            ...
            # Unpack results into normalized, original, and SHARP metadata
        ...
        sharp_metadata = [r[2] for r in results]  # Collect SHARP metadata from all segments
        
        return (timestamp, np.stack(normalized_channels, axis=-1), np.stack(original_channels, axis=-1), sharp_metadata)
    except Exception as e:
...
def process_case_folder(case_folder_path, available_segments):
    try:
        ...
        # Collect SHARP metadata from all timesteps
        sharp_metadata_sequence = [item[3] for item in timestep_data]
        ...
        return {
            ...
            'sharp_metadata': sharp_metadata_sequence  # Add SHARP metadata
        }
    except Exception as e:
...
def load_external_npz(external_npz_path):
    try:
        ...
        # Safely load sharp_metadata, create placeholder if not present
        if 'sharp_metadata' in data:
            sharp_metadata = data['sharp_metadata']
        else:
            print("Warning: 'sharp_metadata' not found in external file. Creating empty placeholder.")
            sharp_metadata = [None] * len(X)
        ...
        if isinstance(sharp_metadata, np.ndarray): sharp_metadata = sharp_metadata.tolist()
        return X, X_original, y, metadata, sharp_metadata
    except Exception as e:
        print(f"Error loading external .npz file: {e}")
        return None, None, None, None, None

def remove_duplicates_by_case(X_curr, X_orig_curr, y_curr, meta_curr, sharp_curr, X_ext, X_orig_ext, y_ext, meta_ext, sharp_ext):
    ...
    if not ext_indices_to_keep:
        ...
        return X_curr, X_orig_curr, y_curr, meta_curr, sharp_curr
    ...
    sharp_ext_filtered = [sharp_ext[i] for i in ext_indices_to_keep]
    ...
    sharp_combined = sharp_curr + sharp_ext_filtered
    ...
    return X_combined, X_orig_combined, y_combined, meta_combined, sharp_combined

def combine_with_external_data(X_curr, X_orig_curr, y_curr, meta_curr, sharp_curr, external_npz_path):
    X_ext, X_orig_ext, y_ext, meta_ext, sharp_ext = load_external_npz(external_npz_path)
    if X_ext is None:
    ...
    X_comb, X_orig_comb, y_comb, meta_comb, sharp_comb = remove_duplicates_by_case(X_curr, X_orig_curr, y_curr, meta_curr, sharp_curr, X_ext, X_orig_ext, y_ext, meta_ext, sharp_ext)
    ...
    return X_comb, X_orig_comb, y_comb, meta_comb, sharp_comb
...
if __name__ == "__main__":
    ...
    if not processed_data:
        ...
        X_final, X_original_final, y_final, metadata, sharp_metadata = create_final_dataset(processed_data)
        if X_final is not None and X_final.size > 0:
            ...
            if COMBINE_EXTERNAL_NPZ:
                ...
                X_final, X_original_final, y_final, metadata, sharp_metadata = combine_with_external_data(X_final, X_original_final, y_final, metadata, sharp_metadata, EXTERNAL_NPZ_FILE)
            ...
            np.savez_compressed(OUTPUT_FILE, X=X_final, X_original=X_original_final, y=y_final, metadata=metadata, sharp_metadata=sharp_metadata)
            ...
            if sharp_metadata and sharp_metadata[0] is not None:
                print(f"\nSample SHARP metadata (first timestep, first segment):")
                sample_sharp = sharp_metadata[0][0][0]  # First case, first timestep, first segment
                for key, value in sample_sharp.items():
                    print(f"  {key}: {value}")
